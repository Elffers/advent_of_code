...
class Seater
  ...
  def score arrangement
    arrangement.flat_map.with_index do |guest, i|
      l_neighbor = arrangement[(i + 1) % arrangement.length]
      r_neighbor = arrangement[i - 1]

      [scores[guest][l_neighbor], scores[guest][r_neighbor]]
    end.reduce(:+)
  end
end
