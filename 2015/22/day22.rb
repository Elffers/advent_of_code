...
class Wizard
  attr_accessor :points, :damage, :mana, :timer, :spent

  def initialize(points:, damage:, mana: 0)
    @points = points
    @damage = damage
    @mana = mana
    @timer = Hash.new 0
  ...
  def cast(spell)
    ...
    if spell.effect && !timer[spell.name]
      timer[spell.name] = spell.effect
    end
...
class Spell
  attr_accessor :cost, :damage, :armor, :effect, :mana, :points

  def initialize(cost:, damage: nil, armor: nil, effect: nil, mana: nil, points: nil)
    @cost = cost
    @damage = damage
    @armor = armor
    @mana = mana
...
SPELLS = {
  "magic":    Spell.new(cost: 53, damage: 4),
  "drain":    Spell.new(cost: 73, damage: 4, points: 2),
  "shield":   Spell.new(cost: 113, effect: 6, armor: 7),
  "poison":   Spell.new(cost: 173, damage: 3, effect: 6),
  "recharge": Spell.new(cost: 229, effect: 5, mana: 101)
}
