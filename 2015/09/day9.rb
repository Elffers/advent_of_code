# --- Day 9: All in a Single Night ---

# Every year, Santa manages to deliver all of his presents in a single night.

# This year, however, he has some new locations to visit; his elves have
# provided him the distances between every pair of locations. He can start and
# end at any two (different) locations he wants, but he must visit each
# location exactly once. What is the shortest distance he can travel to
# achieve this?

# For example, given the following distances:

# London to Dublin = 464 London to Belfast = 518 Dublin to Belfast = 141 The
# possible routes are therefore:

# Dublin -> London -> Belfast = 982 London -> Dublin -> Belfast = 605 London
# -> Belfast -> Dublin = 659 Dublin -> Belfast -> London = 659 Belfast ->
# Dublin -> London = 605 Belfast -> London -> Dublin = 982 The shortest of
# these is London -> Dublin -> Belfast = 605, and so the answer is 605 in this
# example.

# What is the distance of the shortest route?

require 'set'

class Router
  attr_accessor :routes, :cities

  def initialize(input)
    @input = input.map { |line| line.strip! }
    @routes = Hash.new { |h, k| h[k] = {} }
    @cities = Set.new
  end

  def parse
    @input.each do |line|
      /(?<origin>\w+)\sto\s(?<dest>\w+)\s=\s(?<dist>\d+)/ =~ line
      routes[origin][dest] = dist.to_i
      routes[dest][origin] = dist.to_i
      @cities << origin << dest
    end
  end


  def shortest_path
    parse
    paths.min
  end

  def paths
    cities.to_a.permutation.to_a.map do |path|
      path.each_cons(2).map do |origin, dest|
        routes[origin][dest]
      end.reduce(:+)
    end
  end

end

if $0 == __FILE__
  input = File.readlines("day9_input.txt")
  r = Router.new(input)
  r.parse
  p r.shortest_path
  p r.paths.max
end
